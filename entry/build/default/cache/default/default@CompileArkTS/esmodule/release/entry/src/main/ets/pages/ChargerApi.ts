...
export class ChargerApi {
    ...
    static async fetchNearbyStations(context?: common.UIAbilityContext): Promise<StationBrief[]> {
        ...
        try {
            ...
            // 步骤4: 如果仍然失败，使用默认坐标
            if (!locationSuccess) {
                console.log('⚠️ Using default coordinates as fallback');
                userLocation = {
                    latitude: DEFAULT_LATITUDE,
                    longitude: DEFAULT_LONGITUDE
                };
                ...
                if (rawData && rawData.data && rawData.data.elecStationData) {
                    ...
                    for (let i = 0; i < rawData.data.elecStationData.length; i++) {
                        ...
                        if (item.latitude && item.longitude) {
                            distance = ChargerApi.calculateDistance(userLocation.latitude, userLocation.longitude, item.latitude, item.longitude);
